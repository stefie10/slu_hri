...
from scipy import array, zeros, fmod
from pylab import *
import cPickle
...
def compute_log_posterior(obj_name, prior_cond, prior, vpolygons):
    #p(o | v_1 ... v_n) = p(v_1 ... v_n | o)p(o)/p(v_1 ... v_n)
    #=(p(v_1 | o, v_2) ... p(v_n-1 | o, v_n) p(o))/( \pi p(v_1 | v_2) ... p(v_n-1 | v_n) )
    
    #print "computing posterior"
    log_num, log_denom = log(10.0**-6.0), log(10.0**-6.0)
...
def compute_most_likely_locations(obj_name, map_filename, tag_filename, prior_cond):
    ...
    #get_visible_polygons([64.2,32.7], poly, mymap)
    #vpolys = get_visible_polygons([46.4,26.2], poly, mymap)
...
if __name__=="__main__":
    if(len(argv) == 5):
        print "loading prior"
        compute_most_likely_locations(argv[1], argv[2], argv[3], cPickle.load(open(argv[4], 'r')))
    else:
        print "usage:\n\t python flickr_compute_most_likely_locations.py obj_name map_file tag_file prior"


#mymap.to_xy(gsl_vector *ind);
#mymap.is_visible(gsl_vector* pt1, gsl_vector* pt2);
#mymap.ray_trace(double startx, double starty, gsl_vector* thetas);
