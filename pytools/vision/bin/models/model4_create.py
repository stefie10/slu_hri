...
def create_model4(objects_filename, 
    ...
    l_map = likelihood_map_model4(loc_info, flickr_cache, thelogfile)
    ...
    cPickle.dump(l_map, open(outfilename+"_m4.pck", 'w'))

if __name__=="__main__":
    if(len(argv) == 5):
        print "loading prior"
        create_model4(argv[1], cPickle.load(open(argv[2], 'r')),
                      cPickle.load(open(argv[3], 'r')), argv[4])
    else:
        print "usage:\n\t python run_model4.py loc_filename flickr_cache.pck logfile.pck outfileprefix"
