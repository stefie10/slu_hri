...
def create_model6(pclass_log_dir,flickr_cache, thelogfile, outfilename):
    #load a location file
    myfiles = glob(pclass_log_dir+"/*.log")

    print "creating map likelihood"
    l_map = likelihood_map_model6([], flickr_cache, thelogfile)
    ...
    cPickle.dump(l_map, open(outfilename+"_m6.pck", 'w'))

if __name__=="__main__":
    if(len(argv) == 5):
        print "loading prior"
        create_model6(argv[1], cPickle.load(open(argv[2], 'r')),
                      cPickle.load(open(argv[3], 'r')), argv[4])
    else:
        print "usage:\n\t python run_model6.py pclass_log_dir flickr_cache.pck logfile.pck outfileprefix"
