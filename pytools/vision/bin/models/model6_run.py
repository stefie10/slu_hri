...
def run_model6(l_map, outfilename, num_iterations=100):
    ...
    cPickle.dump(l_map, open(outfilename+"_m6_run.pck", 'w'))

if __name__=="__main__":
    if(len(argv) == 3):
        print "loading model"
        run_model6(cPickle.load(open(argv[1], 'r')), argv[2])
    elif(len(argv) == 4):
        print "loading model"
        run_model6(cPickle.load(open(argv[1], 'r')), argv[2], int(argv[3]))
    else:
        print "usage:\n\t python run_model6.py mymodel.pck outfilename [num_iterations]"
