...
def run_model4(l_map, outfilename, num_iterations=100):
...
if __name__=="__main__":
    if(len(argv) == 3):
        print "loading model"
        run_model4(cPickle.load(open(argv[1], 'r')), argv[2])
    elif(len(argv) == 4):
        print "loading model"
        run_model4(cPickle.load(open(argv[1], 'r')), argv[2], int(argv[3]))
    else:
