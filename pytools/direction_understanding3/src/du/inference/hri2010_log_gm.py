...
class hri2010_log_gm:
    ...
    def inference_approx(self):
        ...
        for i in range(len(self.T_seq)):
            #print "update", i
            if(not len(self.SR_seq) == 0 and not self.SR_seq[i] is None):
            ...
            #  a transition
            if(self.O_seq[i] is None):
                O_mat = array([])
            else:
                O_mat = self.O_seq[i]
                
            if(self.D_seq is None or self.D_seq[i] is None):
                D_mat = array([[]])
            else:
                D_mat = self.D_seq[i]
...
#        print "P_prev_log",P_prev_log
        ...
        if(curr_best_path is None or l_p_best == log(0)):
            return [], l_p_best
