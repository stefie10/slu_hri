...
class App:
    def __init__(self, m4du, rndf_file, inference_type="local",num_explorations=None,exploration_heuristics_name=None,parameters=None):
        self.lc = lcm.LCM()
    ...
    def publish_waypoints(self):
        ...
        if self.run_inference:
            ...
            print "Number of sdcs:", len(keywords)
            print "Actual SDCs: ", keywords
        
            if self.inference_type=="local":
                (best_path, lprob, sdc_eval), visited_vps = infer_explore(self.dg_model, keywords, self.curr_location[0:2], [self.curr_orientation], self.num_explorations, self.exploration_heuristics_name, self.params_num)       
                self.waypoints = []
                print "WAY TO GO: ", best_path #visited_vps
                way_to_go = best_path
            elif self.inference_type=="global" :
                best_path, lprob, sdc_eval = self.dg_model.infer_path(keywords, self.curr_location[0:2], [self.curr_orientation])
                self.waypoints = []
                way_to_go = best_path
                print "WAY TO GO: ", way_to_go
            
            for myreg in way_to_go:
                #myreg might be viewpoint or topo
                topo= str(myreg).split("_")[0]
...
#                    self.waypoints.insert(min_idx, pt)
        ...
        if len(remaining_trans) and remaining_trans[0] in self.transitions:
            ...
            self.chkpoint = 0
        print "checkpoint", self.chkpoint
        ...
        print "checkpoints:", remaining_trans
...
def main():
    ...
    parser = OptionParser(usage="usage:python lcm_networking_forklift.py [options] model_filename.pck rndf_modelname.rndf ")
    parser.add_option("--inference_type", type="string")
